<template>
  <v-dialog v-model="auth.show" fullscreen transition="dialog-bottom-transition">
    <v-card>
      ...
      <v-stepper v-model="auth.step" vertical>
        <v-stepper-step step="1" :complete="auth.step > 1">
        ...
        <v-stepper-content step="1">
          ...
          <v-layout>
            <v-btn :disabled="line.isOnline" :loading="line.isConnecting" color="primary" @click="connect()">Connect</v-btn>
          </v-layout>
        </v-stepper-content>

        <v-stepper-step step="2" :complete="auth.step > 2">
        ...
        <v-stepper-step step="3" :complete="auth.step > 3">Introduce Yourself</v-stepper-step>
...
export default {
  ...
  methods: {
    ...mapMutations(['updateLine', 'hideAuth', 'updateAuthStep']),
    cancel() {
      this.hideAuth();
      this.disconnect();
    },
    ...
    disconnect() {
      ...
      this.channel = null;
      this.user = null;
      this.updateAuthStep({ step: 1 });
      localStorage.removeItem('auth');
    },
    generate() {
      this.line.channel = this.channel;
      this.updateAuthStep({ step: 3 });
